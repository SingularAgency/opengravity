...
async function callPromptsApi(method: string, params: any) {
...
export async function searchSkillsFn(args: SkillSearchParams) {
    try {
        ...
        if (skills.length === 0) return "No skills found for that query.";

        return skills.map((p: any) =>
...
export async function getSkillFn(args: SkillGetParams) {
    try {
        const data = await callPromptsApi("tools/call", {
            name: "get_skill",
            arguments: {
        ...
        if (data.error) return `Skill not found: ${data.error}`;

        const filesInfo = (data.files || []).map((f: any) => `- ${f.filename}`).join("\n");
...
export async function installSkillFn(args: { id: string; slug: string }) {
    try {
        const data = await callPromptsApi("tools/call", {
            name: "get_skill",
            arguments: {
